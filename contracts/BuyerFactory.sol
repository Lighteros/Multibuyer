...
interface IBuyer {
    ...
    function buyV2WithTax(
        address router,
        address[] calldata path
    ) external payable;
    function buyV3ExactInput(
        address router,
        uint256 amountIn,
        address tokenIn,
        address tokenOut,
...
contract BuyerFactory is Ownable {
    ...
    constructor(address _buyer, uint256 preClonesNum) Ownable(msg.sender) {
        ...
        for (uint256 i = 0; i < preClonesNum; i++) {
            ...
            IBuyer(identicalChild).initialize(msg.sender);
    ...
    function batchBuyV2(
        ...
        for (uint256 i = 0; i < walletNum; i++) {
            IBuyer(preClones[i]).buyV2{value: ethPerWallet}(
                router,
                amountOut,
                path
            );
    ...
    function batchBuyV2WithTax(
        ...
        for (uint256 i = 0; i < walletNum; i++) {
            IBuyer(preClones[i]).buyV2WithTax{value: ethPerWallet}(
                router,
                path
            );
        }
    ...
    function batchBuyV3(
        ...
        for (uint256 i = 0; i < walletNum; i++) {
            IBuyer(preClones[i]).buyV3{value: ethPerWallet}(
                router,
                amountOut,
                tokenIn,
                tokenOut,
    ...
    function batchBuyV3ExactInput(
        ...
        for (uint256 i = 0; i < walletNum; i++) {
            IBuyer(preClones[i]).buyV3ExactInput{value: ethPerWallet}(
                router,
                amountIn,
                tokenIn,
                tokenOut,
